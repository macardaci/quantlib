...
namespace {
    ...
    struct CommonVars {
        ...
        vector<shared_ptr<SimpleQuote> > rates, fraRates,
                                                     ...
                                                     prices, fractions;
        vector<shared_ptr<BootstrapHelper<ForwardRateCurve> > > instruments, fraHelpers,
                                                    immFutHelpers, asxFutHelpers,
                                                    bmaHelpers;
        vector<shared_ptr<BootstrapHelper<YieldTermStructure> > > bondHelpers;
        ...
        CommonVars() {
            ...
            bmaHelpers = vector<shared_ptr<BootstrapHelper<ForwardRateCurve>> >(bmas);
    ...
    void testBMACurveConsistency(CommonVars& vars,
        ...
        for (Size i=0; i<vars.bmas; ++i) {
            ...
            vars.bmaHelpers[i] = shared_ptr<BootstrapHelper<ForwardRateCurve>>(
        ...
        // check BMA swaps
        shared_ptr<BMAIndex> bma(new BMAIndex(curveHandle));
        shared_ptr<IborIndex> libor3m(new USDLibor(3*Months,
                                                          riskFreeCurve));
        for (Size i=0; i<vars.bmas; i++) {
            ...
            swap.setPricingEngine(shared_ptr<PricingEngine>(new
                DiscountingSwapEngine(convertIntoYTSHandle(libor3m->forwardingTermStructure()))));
